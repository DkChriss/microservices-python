...
from schemas.request import RequestResponse, RequestUpdate, RequestStore
...
def list (
    ...
    try:
        ...
        if search:
            query = query.filter(
                or_(
                    Request.address.like(f'%{search}%'),
                    Request.wifi.like(f'%{search}%'),
                    Request.location.like(f'%{search}%'),
                )
            )
