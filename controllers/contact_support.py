...
from schemas.contact_support import ContactSupportResponse, ContactSupportUpdate, ContactSupportStore
...
def list (
    ...
    try:
        ...
        if search:
            query = query.filter(
                or_(
                    ContactSupport.name.like(f'%{search}%'),
                    ContactSupport.email.like(f'%{search}%'),
                    ContactSupport.title.like(f'%{search}%'),
                    ContactSupport.message.like(f'%{search}%'),
                )
            )
